...
func (h *Handler) Detect(path string) (*plugins.DetectResult, error) {
	...
	if ext != ".bblx" {
		return &plugins.DetectResult{Detected: false, Reason: "not a .bblx file"}, nil
	}

	return &plugins.DetectResult{
		Detected: true,
		Format:   "esword",
		Reason:   "e-Sword Bible file detected",
	}, nil
}
...
func (h *Handler) ExtractIR(path, outputDir string) (*plugins.ExtractIRResult, error) {
	return nil, fmt.Errorf("esword format does not support IR extraction")
}
...
func (h *Handler) EmitNative(irPath, outputDir string) (*plugins.EmitNativeResult, error) {
	return nil, fmt.Errorf("esword format does not support native emission")
}
